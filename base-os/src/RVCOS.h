...
typedef struct {
  uint32_t DPalette : 2;  // whic palette to use 0 1 2 3
  uint32_t DXOffset : 10;
  uint32_t DYOffset : 9;
  uint32_t DWidth : 4;
  uint32_t DHeight : 4;
  uint32_t DZ : 3;
} SSmallSpriteControl, *SSmallSpriteControlRef;  // 128 of these exist globally
typedef struct {
  uint32_t DPalette : 2;
  uint32_t DXOffset : 10;
...
typedef struct {
  ...
  // this is probably wrong. need a list of held mutexes idk syntax for that
